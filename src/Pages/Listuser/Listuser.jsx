...
import { onSnapshot, collection, doc, updateDoc, increment, decrement } from 'firebase/firestore';
...
const Listuser = () => {
    ...
    const [userCounts, setUserCounts] = useState({ totalCount: 0, employeeCount: 0, approverCount: 0, managerCount: 0 });
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, 'users'), (snapshot) => {
            ...
            setUsersList(usersList);
              // Tính toán số lượng người dùng theo loại tài khoản
      const counts = countUsers(usersList);
    ...
    const handleDelete = () => {
       ...
       // Cập nhật số lượng người dùng sau khi xóa
 ...
 // Hàm tính toán số lượng người dùng theo loại tài khoản
 const countUsers = (usersList) => {
    ...
    // Đếm số lượng người dùng cho từng loại tài khoản
  ...
  // Hàm cập nhật số lượng người dùng sau khi xóa
  const updateCountAfterDelete = async (deletedRows) => {
    try {
      ...
      // Đếm số lượng người dùng còn lại sau khi xóa
      usersList.forEach((user) => {
      ...
      // Cập nhật số lượng người dùng trên Firebase
